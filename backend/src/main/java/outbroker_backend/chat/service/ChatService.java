...
public class ChatService {
    ...
    public ChatRoomResponse getOrCreateRoom(UUID propertyId, UUID currentUserId) {
        ...
        if (landlordId.equals(currentUserId)) {
            throw new IllegalArgumentException("Property owners cannot open a tenant chat with themselves");
        }

        ChatRoom room = chatRoomRepository.findByPropertyIdAndTenantIdAndLandlordId(propertyId, tenantId, landlordId)
    ...
    public List<ChatRoomResponse> getUserRooms(UUID userId) {
        return chatRoomRepository.findAllUserChatRooms(userId).stream()
    ...
    public ChatMessageResponse saveMessage(UUID senderId, ChatMessageRequest request) {
        ...
        if (!room.getTenantId().equals(senderId) && !room.getLandlordId().equals(senderId)) {
            throw new UnauthorizedAccessException("You are not a participant in this chat room");
        }
    ...
    public Page<ChatMessageResponse> getRoomMessages(UUID roomId, UUID userId, Pageable pageable) {
        ...
        if (!room.getTenantId().equals(userId) && !room.getLandlordId().equals(userId)) {
